...
chrome.browserAction.onClicked.addListener(function(tab) {
  ...
  if (sessionId == null) {
    ...
    $.ajax({
      ...
      url: serverIP + "/api/external_documents/name/" + encodeURIComponent(tab.title.replace(/[:"/\&<>#'`%_]/g,'')) + "?session_id=" + sessionId,
      success: function (data) {
        chrome.tabs.executeScript({
          code: "window.getSelection().toString();"
        }, function(selection) {
          addDocumentReference(tab.title.replace(/[:"/\&<>#'`%_]/g,''),String(selection[0]));
        });
      },
      error: function (xhr, textStatus, errorThrown) {
        if (xhr.status == 404) {
